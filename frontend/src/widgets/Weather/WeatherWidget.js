...
class WeatherWidget extends React.PureComponent {
	...
	componentWillMount() {
		...
		script.src =
			'https://maps.googleapis.com/maps/api/js?key=AIzaSyDDXOEh-jzoS5www_Yg5GpIa5ahA1VuKKg&libraries=places';
		script.async = true;
	...
	componentDidMount() {
		...
		this.timerID = setInterval(() => this.setTime(), 20000);
	}
		...
		cities.forEach(city => {
			...
			promises.push(
				axios.get('http://api.openweathermap.org/data/2.5/forecast', {
					params: { q: city, appid: apiKey, units: units, cnt: resultCount },
				}),
			);
		});
										...
										<div className="row widget__footer widget__footer--dark clearfix no-padding">
											<div className="col-md-7 col-sm-6 col-xs-12">
												<h3 className="weather__temperature">
													{Math.round(city.data.list[0].main.temp)}
													<span>o</span>
												</h3>
												<h4 className="weather__wind">
													<i className="wi wi-strong-wind" />
													{city.data.list[0].wind.speed} m/s
												</h4>
											</div>
...
const displayWeatherIcon = icon => {
	...
	return <i className={`wi ${weatherIcon}`} />;
};
